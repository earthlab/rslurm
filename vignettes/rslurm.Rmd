...
single machine with `mclapply` or `mcMap` (from the [`parallel`](https://cran.r-project.org/package=parallel) package).
...
session without direct access to the cluster, you can set `submit = FALSE`. 
Either way, the function creates a folder called `\_rslurm\_[jobname]` in the 
working directory that contains scripts and data files, but nothing is sent to 
the cluster with `submit = FALSE`. In the latter case, the generated folder may
be moved to a SLURM head node, the shell command `sbatch submit.sh` run from
within the folder, and the folder copied back with new `results_*.RDS` files
(one for each node):

```{r}
dir('_rslurm_test_apply')
```

Having now been run on the cluster, the results from all the nodes can be read
back into R with the `get_slurm_out()` function. Note that, if the job was
submitted manually, the job ID must also be added manually.

```{r}
sjob$jobid <- 14283 # unnecessary with `submit = TRUE` in `slurm_apply`
