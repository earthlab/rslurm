...
#' Print the status of a SLURM job and, if completed, its console/error output.
#'
#' If the specified SLURM job is still in the queue or running, this function
#' prints its current status (as output by the SLURM \code{squeue} command). If it 
#' has completed or stopped, the function prints the console and error output, 
#' if any, generated by each node.
#' 
#' @param slr_job A \code{slurm_job} object output by \code{\link{slurm_apply}}.
...
print_job_status <- function(slr_job) {
  ...
  if (length(stat) > 1) {
    print(c('Job still running. Status:', stat))
  } else {
    print('Job completed. Printing console output below if any.')
